...
function initializeWordJumble() {
  const words = [
    { word: "வருடம்", scrambled: "முற்டவ" },
    { word: "சூரியன்", scrambled: "ரிசுனய" },
    { word: "நீர்", scrambled: "ரீன்" },
    { word: "பூமி", scrambled: "ம்பூி" },
    { word: "அக்னி", scrambled: "க்இனா" },
  ];

  const container = document.getElementById("scramble-container");
  container.innerHTML = "";

  words.forEach(({ word, scrambled }) => {
    const div = document.createElement("div");
    div.classList.add("scramble-word");
    div.innerHTML = ` 
      <p>சொல்: ${scrambled}</p>
      <input type="text" placeholder="சரி சொல் உள்ளிடவும்" />
      <button onclick="checkScramble('${word}', this)">சரிபார்க்கவும்</button>
    `;
    container.appendChild(div);
  });
}

function checkScramble(correctWord, button) {
  const input = button.previousElementSibling;
  if (input.value === correctWord) {
    alert("சரியானது!");
  } else {
    alert("தவறு! மீண்டும் முயற்சிக்கவும்.");
  }
}
